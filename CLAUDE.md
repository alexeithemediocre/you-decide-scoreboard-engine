...
- **Browser caveat:** opening the pages via file:// only syncs in Chrome/Edge (all file:// pages share one origin there). Firefox (since 68) and Safari isolate each local file's storage, so nothing syncs — the pages must be served from http://localhost instead. `Start scoreboard.command` (double-clickable, requires Python 3 and explains itself if it's missing) starts `python3 -m http.server 8123` and opens the control room; both control pages' footer hints mention it.
