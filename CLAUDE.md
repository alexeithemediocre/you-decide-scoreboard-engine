...
- **Browser caveat:** opening the pages via file:// only syncs in Chrome/Edge (all file:// pages share one origin there). Firefox (since 68) and Safari isolate each local file's storage, so nothing syncs — the pages must be served from http://localhost instead. `Start scoreboard.command` (macOS) and `Start scoreboard.bat` (Windows; CRLF endings enforced via .gitattributes) are double-clickable, require Python 3 and explain themselves if it's missing; both start `python -m http.server 8123` and open the control room. Both control pages' footer hints mention them.
